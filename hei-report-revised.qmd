...
In @tbl-bp-med we show estimates of the CDEs for different sets of potential mediators. \comment{OC 2d} The first column shows the total effect of the CHP on blood pressure outcomes (e.g., a 1.6 mmHg decrease in central DBP). The second column shows the CDE, i.e., the effect of exposure (vs. no exposure) to the CHP on blood pressure, in a counterfactual population where we intervene to fix the value of indoor PM~2.5~ to the average value for untreated participants at baseline. This CDE is still -1.4 (95% CI -3.0, 0.0) not particularly different from the total effect of -1.6, demonstrating that, under the assumptions outlined in Section 5.2, little of the total effect is mediated by the impact of the policy on indoor PM~2.5~. On the other hand, the estimated CDE when holding constant the value of indoor temp to that of the untreated participants was smaller (-1.1, 95% CI: -2.3, 0.1) suggesting a somewhat stronger role for indoor temperature in mediating the total effect of the CHP on central DBP. Finally, the last two columns show an even further reduced CDE of -0.9 (95% CI: -2.3, 0.5) when setting both indoor PM~2.5~ and indoor temperature to their respective pre-treatment means. Thus, interventions to fix the values of indoor PM~2.5~ and indoor temperature to the levels in the untreated group at baseline would explain roughly 44% of the total effect of the CHP on central DBP. Overall the results in @tbl-bp-med indicate that conditioning on indoor PM~2.5~ and indoor temperature largely explains the entire total effect of the CBHP policy on blood pressure for systolic BP, as the CDE conditional on both mediators was reduced to -0.0 (95%CI: -2.0, 2.0) for brachial SBP. The CDE for diastolic BP was roughly half the value of the total effect. 
...
tt(otm,
   digits = 1,
   #width = c(3.5, 3, 1, 0.5, 2, 0.5, 2, 0.5, 2, 0.5, 2),
   notes = list("Note: Results combined across 30 multiply-imputed datasets. ATT = Average Treatment Effect on the Treated, CDE = Controlled Direct Effect, CI = Confidence Interval, DBP = Diastolic blood pressure, SBP = Systolic blood pressure.", a = list(i=0, j=2, text = "Adjusted for age, sex, waist circumference, smoking, alcohol consumption, and use of blood pressure medication."), b = list(i=0, j=c(4,6,8), text = "Mediators were set to the mean value for untreated participants at baseline."))) %>%
  group_tt(
