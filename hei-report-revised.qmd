...
tt(m_table,
   digits = 2,
  #width = c(3.5, 3, 1, 0.5, 2, 0.5, 2, 0.5, 2, 0.5, 2),
  notes = list("Note: ATT = Average Treatment Effect on the Treated, DiD = Difference-in-Differences.", 
     a = list(i=0, j=6,
     text = "Extended two-way fixed effects models for air pollution outcomes were adjusted for household size, smoking, outdoor temperature, and outdoor dewpoint. Temperature models not additionally adjusted."))) %>%
  group_tt(
    j = list("DiD" = 4:5, 
             "Adjusted DiD" = 6:7),
    i = list("Air pollution (µg/m3)" = 1, 
             "Indoor temperature (°C)" = 7)) %>%
...
bp_policy_t <- tibble(
  category = rep(c("Systolic BP (mmHg)", "Diastolic BP (mmHg)", 
