...
att_did_gaussian <- att_table(preds = bme_pred, 
  meffs = bme_avg)
...
# wrangle aggregate ATTs for model summary table
att_did_skew <- att_table(preds = bme_pred, 
  meffs = bme_avg)
...
# wrangle aggregate ATTs for model summary table
att_did_log_m <- att_table(preds = bme_pred, 
  meffs = bme_avg)

# compare posterior predictive checks
ppc_pe_g <- pp_check(pe_gaussian, ndraws = 100) +
  theme_classic() + xlim(-250, 1000) +
  scale_color_manual(name = "Gaussian", values=c("#e41a1c", "#377eb8"),
    labels = c("Observed data", "Posterior draws")) +
  xlab("Personal PM2.5") + 
  theme(legend.position = c(0.7, 0.5))
  
ppc_pe_s <- pp_check(pe_skew, ndraws = 100) + 
   theme_classic() + xlim(-250, 1000) +
  scale_color_manual(name = "Skewnormal", values=c("#e41a1c", "#377eb8"),
    labels = c("Observed data", "Posterior draws")) +
  xlab("Personal PM2.5") + 
  theme(legend.position = c(0.7, 0.5))

ppc_pe_l <- pp_check(pe_log, ndraws = 100) + 
  theme_classic() + xlim(-250, 1000) +
  scale_color_manual(name = "Lognormal", values=c("#e41a1c", "#377eb8"),
    labels = c("Observed data", "Posterior draws")) +
  xlab("Personal PM2.5") + 
  theme(legend.position = c(0.7, 0.5))

ppc_pe_g + ppc_pe_s + ppc_pe_l +
...
# wrangle aggregate ATTs for model summary table
att_did_gauss_log <- att_table(preds = bme_pred, 
  meffs = bme_avg)

feols_pred <- predictions(
  pe_feols_log, 
  newdata   = subset(d_personal, treat==1),
  variables = "treat", 
  by        = "treat", transform = exp,
  )

feols_atts <- get_estimates(feols_pred) %>%
  mutate(se = abs(conf.high - conf.low) / (2 * 1.96)) %>%
  select(treat, estimate, se) %>%
  pivot_wider(names_from = 
    treat, values_from = c(estimate, se)) %>%
  mutate(
    estimate_2 = estimate_1 - estimate_0,
    conf.low_0 = estimate_0 - 1.96 * se_0,
    conf.high_0 = estimate_0 + 1.96 * se_0,
    conf.low_1 = estimate_1 - 1.96 * se_1,
    conf.high_1 = estimate_1 + 1.96 * se_1,
    se_2 = sqrt(se_1^2 + se_0^2),
  ...
  mutate(term = case_when(
    ...
    term == 1 ~ 2,
    term == 2 ~ 3),
    term = factor(term, labels = c("Untreated", 
    "Treated", "Difference")))

gl2 <- data.frame()

att_did_feols <- list(tidy = data.frame(feols_atts), 
 glance = gl2)

class(att_did_feols) <- "modelsummary_list"
...
modelsummary(list(
  ...
  "Gauss (log)" = att_did_gauss_log,
  "FE GLM" = att_did_pe_feglm), 
  fmt=1, 
...
est_pe_log_flat <- get_estimates(pe_log_flat) %>%
...
brms_estimates_pe_f <- list(tidy = est_pe_log_flat, glance = gl2)

class(brms_estimates_pe_f) <- "modelsummary_list"

modelsummary(list(
  "BRMS" = brms_estimates_pe,
  "BRMSF" = brms_estimates_pe_f,
  "FE GLM" = pe_log_feglm,
  "GLM" = pe_logn_glm),
  coef_rename = 
...
pe_log_post <- as_draws_df(pe_log)
pe_log_post %>% 
  ggplot(aes(x = prior_b)) + 
