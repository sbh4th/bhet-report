...
```{r setup, include=FALSE, cache=TRUE, message=FALSE, warning=FALSE, eval=F}
library(here)
library(osfr)
library(tidyverse)
library(readxl)
...
Another DiD table
```{r ap-did-table, echo=F, message=F, warning=F}
library(tidyverse)
library(here)
library(kableExtra)
...
# join tables
o_table <- bind_rows(ap_table, r_table)

kable(o_table,  digits = 2,
  ...
  pack_rows("Respiratory (percentage points)", 7, 12) %>%
  add_header_above(c(" " = 2, 
...
```{r bp-resp-table, echo=F, message=F, warning=F}
library(tidyverse)
library(here)
library(kableExtra)
...
# join tables
op_table <- bind_rows(bp_policy_t, r_table, feno_table)
